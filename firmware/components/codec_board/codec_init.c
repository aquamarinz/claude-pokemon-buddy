...
static int _i2c_init(uint8_t port)
{
    ...
    // pokemon-buddy: our SHTC3 (B3) already created the master bus on this port,
    // and SHTC3 + ES8311 physically share SDA13/SCL14 — a port can only be
    // created once. Reuse the existing bus instead of failing on a duplicate
    // i2c_new_master_bus.
    {
