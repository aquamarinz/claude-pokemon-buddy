...
test("教程文案覆盖 spec 要求的三个主题：KEY 两种按法、BOOT、用量表", () => {
...
- Test: `host/test/main-orchestration.test.js`（更新 RH3 用例：mock 补教程按键、订阅数 2→3）
...
- [ ] **Step 3b: 更新 `host/test/main-orchestration.test.js` 的 RH3 用例**（main() 接线教程后，原 mock 按键序列停在诞生屏会挂死）

`createOnboardingTransport()` 的 `push()` 分支（约 :740）追加一条——诞生屏（第 `OAK_LINES.length + 14` 次 push）之后的下一次 push 是教程首屏，发长按跳过：
...
```bash
git add host/src/index.js host/test/onboarding-gate.test.js host/test/main-orchestration.test.js
...
- Produces: CLI `node cpb-statusline-fanout.mjs --b64 <base64(原command)>`（推荐，免引号转义地狱）或 `node cpb-statusline-fanout.mjs <原command...>`——stdin 完整喂 bridge（丢弃其 stdout）与原 command（shell 方式执行，stdout 透传为本进程输出）；原 command 失败/缺省 → 退回 bridge 的一行输出；永不抛错、退出码恒 0。SETUP-WINDOWS.md（Task 6）的共存分支引用 `--b64` 用法。
